...
class AsignacionAutomatica {
    ...
    private $disponiblesCache = [];
    private $disponiblesConFallbackCache = [];
    private $conteoDisponiblesCache = [];
    private $bloqueadosAdmCache = [];
    private $restriccionPuestoEspecificoEnFechaCache = [];
    ...
    private function getDisponibles($puestoId, $turnoId, $fecha, &$ctx, &$conteoTurnos) {
        $cacheKey = $puestoId . '|' . $turnoId . '|' . $fecha;
        if (isset($this->disponiblesCache[$cacheKey])) {
            $disponibles = $this->disponiblesCache[$cacheKey];
        } else {
            $disponibles = $this->trabajadores->obtenerDisponibles($puestoId, $turnoId, $fecha);
            $this->disponiblesCache[$cacheKey] = $disponibles;
        }
    ...
    private function getDisponiblesConFallback($puestoId, $turnoId, $numeroTurno, $fecha, &$ctx, &$conteoTurnos) {
        $disponibles = $this->getDisponiblesRawConFallback($puestoId, $turnoId, $numeroTurno, $fecha, $ctx, $conteoTurnos, 'normal');
    ...
    private function getDisponiblesRawConFallback($puestoId, $turnoId, $numeroTurno, $fecha, &$ctx, &$conteoTurnos, $modo = 'normal') {
        $cacheKey = $puestoId . '|' . $turnoId . '|' . $numeroTurno . '|' . $fecha . '|' . $modo;
        if (isset($this->disponiblesConFallbackCache[$cacheKey])) {
            return $this->disponiblesConFallbackCache[$cacheKey];
        }

        $bloqueadosAdm = $this->obtenerBloqueadosAdmParaFecha($fecha, $ctx);
        $filtrarAdm = function($lista) use ($bloqueadosAdm) {
            ...
            return array_values(array_filter($lista, function($t) use ($bloqueadosAdm) {
                return !in_array($t['id'], $bloqueadosAdm, true);
            }));
        };

        if ($modo === 'normal') {
            $lista = $filtrarAdm($this->getDisponibles($puestoId, $turnoId, $fecha, $ctx, $conteoTurnos));
            return $this->disponiblesConFallbackCache[$cacheKey] = $lista;
        }

        $lista = $this->trabajadores->obtenerDisponiblesRelajado($puestoId, $turnoId, $fecha, $modo);
        $lista = $filtrarAdm($lista);
        return $this->disponiblesConFallbackCache[$cacheKey] = $lista;
    }

    private function obtenerBloqueadosAdmParaFecha($fecha, &$ctx) {
        $cacheKey = $fecha;
        if (isset($this->bloqueadosAdmCache[$cacheKey])) {
            return $this->bloqueadosAdmCache[$cacheKey];
        }

        $bloqueadosAdm = [];
        foreach ($ctx['diasEspeciales'] as $de) {
            if (!isset($de['tipo'])) continue;
            if (!in_array($de['tipo'], ['ADMM', 'ADMT'], true)) continue;
        ...
        if (!empty($ctx['asignadosPorDia'][$fecha])) {
            foreach ($ctx['asignadosPorDia'][$fecha] as $trabId => $turnos) {
                if (in_array('ADMM', $turnos, true) || in_array('ADMT', $turnos, true)) {
        ...
        return $this->bloqueadosAdmCache[$cacheKey] = $bloqueadosAdm;
    }

    private function contarDisponiblesPorPuestoTurno($puestoId, $turnoIdBase, $turnoNumero, $fecha, &$ctx, &$conteoTurnos) {
        $cacheKey = $puestoId . '|' . $turnoIdBase . '|' . $turnoNumero . '|' . $fecha;
        if (isset($this->conteoDisponiblesCache[$cacheKey])) {
            return $this->conteoDisponiblesCache[$cacheKey];
        }

        $raw = $this->getDisponiblesRawConFallback($puestoId, $turnoIdBase, $turnoNumero, $fecha, $ctx, $conteoTurnos);
        $disponibles = $this->filtrarDisponiblesPorRestriccionPuestoEspecifico($raw, $puestoId, $fecha, $ctx);
        return $this->conteoDisponiblesCache[$cacheKey] = count($disponibles);
    }
    ...
    private function tieneRestriccionPuestoEspecificoEnFecha($trabajadorId, $puestoId, $fecha, &$ctx) {
        ...
        $cacheKey = $trabajadorId . '|' . $puestoId . '|' . $fecha;
        if (isset($this->restriccionPuestoEspecificoEnFechaCache[$cacheKey])) {
            return $this->restriccionPuestoEspecificoEnFechaCache[$cacheKey];
        }
        ...
        foreach ($ctx['restricciones'] as $r) {
            ...
            return $this->restriccionPuestoEspecificoEnFechaCache[$cacheKey] = true;
        }

        return $this->restriccionPuestoEspecificoEnFechaCache[$cacheKey] = false;
    }
    ...
    private function filtrarDisponiblesPorRestriccionesObligatorias($disponibles, $puestoId, $turnoOpciones, $fecha, $excludeId = null) {
        if (empty($disponibles)) {
            return [];
        }
        ...
        foreach ($disponibles as $trabajador) {
            ...
            if ($this->trabajadorEsElegibleParaOpcionesTurno($trabajadorId, (int)$puestoId, $turnoOpciones, $fecha, $excludeId)) {
    ...
    private function trabajadorEsElegibleParaOpcionesTurno($trabajadorId, $puestoId, $turnoOpciones, $fecha, $excludeId = null) {
        if (!is_array($turnoOpciones) || empty($turnoOpciones)) {
            return false;
        }

        foreach ($turnoOpciones as $opcion) {
            $turnoId = (int)($opcion['id'] ?? 0);
            if ($turnoId <= 0) {
                continue;
            }
            if ($this->validarCandidatoAutomatico($trabajadorId, $puestoId, $turnoId, $fecha, $excludeId)) {
                return true;
            }
        }
    ...
    public function asignarMesCompleto($mes, $anio, $opciones = []) {
        ...
        try {
            ...
            if ($modoRapido) {
                $warnings[] = 'Modo rápido activado: se prioriza completar puestos y L4 con menos reordenamientos y sin optimización final; los días libres siguen con límite de tiempo.';
            } elseif ($forzarLlenado) {
                $warnings[] = 'Modo forzar llenado activado: se prioriza cubrir puestos y L4, pero se mantiene optimización final para mejorar cobertura.';
            }
            ...
            // Objetivo MENSUAL para 6 semanas (aproximadamente 42*6=252 horas)
            // Los objetivos semanales (3x8h, 2x7h, 1x4h) se usan como guía en elegirOpcionTurnoParaPerfil
            $perfilObjetivo = [
                'max_horas'    => 252.0,
                'max_8h'       => 18,
                'max_7h'       => 12,
                'max_4h'       => 6,
                // Objetivos semanales para guiar la selección
                'semanal_horas' => 42.0,
                'semanal_8h'    => 3,
                'semanal_7h'    => 2,
                'semanal_4h'    => 1,
            ...
            usort($trabajadoresShuffled, function($a, $b) {
                return strcmp((string)($a['nombre'] ?? ''), (string)($b['nombre'] ?? ''));
            });
            $semanasShuffled = $semanas;
            usort($semanasShuffled, function($a, $b) {
                return strcmp((string)($a['lunes'] ?? ''), (string)($b['lunes'] ?? ''));
            });

            // ════════════════════════════════════════════════════════════════
            // PASO 1 — TURNOS L4
            // Se prioriza de forma limitada para no volver a colapsar el proceso,
            // pero se intenta siempre respetando la meta semanal de 42 horas.
            // ════════════════════════════════════════════════════════════════
            ...
            foreach ($semanasShuffled as $semana) {
                // Commit cada semana para evitar lock timeout
                if ($semaIdx > 0 && $manejaTransaccion && $this->db->inTransaction()) {
                    $this->db->commit();
                    error_log('[COMMIT] Commit después de semana L4 #' . $semaIdx);
                    $this->db->beginTransaction();
                }
                $semaIdx++;
                
                $this->verificarCancelacion('paso L4');
                if ((microtime(true) - $tiempoInicioL4) > $tiempoLimiteL4) {
                    $warnings[] = 'Se interrumpió la asignación de L4 por límite de tiempo de la fase inicial.';
                    break;
                }

                error_log('[PASO 1 L4] Revisando semana ' . ($semana['lunes'] ?? '') . ' con ' . count($trabajadoresShuffled) . ' trabajadores');
                foreach ($trabajadoresShuffled as $trab) {
                    ...
                    $perfilSemana = $perfilSemanal[$trab['id']][$semana['lunes']] ?? ['total_horas' => 0.0];
                    error_log('[PASO 1 L4] Trab=' . $trab['id'] . ' semana=' . ($semana['lunes'] ?? '') . ' horas=' . ((float)($perfilSemana['total_horas'] ?? 0.0)) . ' tieneL4=' . ($this->tieneTurnoL4EnSemana($trab['id'], $semana['lunes'], $semana['domingo'], $turnosPorTrabajadorSemana) ? '1' : '0'));
                    if ((float)($perfilSemana['total_horas'] ?? 0.0) >= 42.0) {
                        continue;
                    }
                        ...
                        foreach ($puestosL4Mezclados as $puesto) {
                            ...
                            foreach ($turnoIdsL4 as $turnoIdL4) {
                                ...
                                if ($resultado['success']) {
                                    $L4AsignacionesExitosas++;
                                    error_log('[PASO 1 L4] ✅ L4 Asignado: trab=' . $trab['id'] . ', puesto=' . $puesto['codigo'] . ', fecha=' . $fechaL4 . ', turno=' . $numeroTurnoL4);
            ...
            error_log('[PASO 1 L4] ✓ Fase L4 completada. Intentadas=' . $L4AsignacionesIntentadas . ', Exitosas=' . $L4AsignacionesExitosas . ', tiempo=' . number_format(microtime(true) - $tiempoInicioL4, 2) . 's');
            ...
            for ($dia = 1; $dia <= $diasMes; $dia++) {
                // Commit DIARIO para evitar lock timeout
                if ($dia > 1 && $manejaTransaccion && $this->db->inTransaction()) {
                    $this->db->commit();
                    error_log('[COMMIT] Commit después de día ' . ($dia-1));
                    $this->db->beginTransaction();
                }
                ...
                // Para maximizar velocidad, se evita reordenar el conjunto de puestos
                // cada día y se usa un orden estático simple por prioridad.
                $puestosOrdenados = [];
                foreach ($puestos as $p) {
                    $cod = strtoupper((string)($p['codigo'] ?? ''));
                    if (in_array($cod, $puestosCriticos, true)) {
                        $puestosOrdenados[] = $p;
                    }
                }
                foreach ($puestos as $p) {
                    $cod = strtoupper((string)($p['codigo'] ?? ''));
                    if (in_array($cod, $puestosPrioridadT1, true) && !in_array($cod, $puestosCriticos, true)) {
                        $puestosOrdenados[] = $p;
                    }
                }
                foreach ($puestos as $p) {
                    $cod = strtoupper((string)($p['codigo'] ?? ''));
                    if (in_array($cod, $puestosPrioridadT2, true) && !in_array($cod, $puestosCriticos, true) && !in_array($cod, $puestosPrioridadT1, true)) {
                        $puestosOrdenados[] = $p;
                    }
                }
                foreach ($puestos as $p) {
                    $cod = strtoupper((string)($p['codigo'] ?? ''));
                    if (!in_array($cod, $puestosCriticos, true) && !in_array($cod, $puestosPrioridadT1, true) && !in_array($cod, $puestosPrioridadT2, true)) {
                        $puestosOrdenados[] = $p;
                    }
                }
            ...
            if ($modoRapido) {
                ...
                $warnings[] = 'Optimización final omitida por modo rápido.';
            } else {
            ...
            // Rebalancear perfil semanal si es obligatorio
            if (!empty($opciones['perfil_obligatorio'])) {
                $this->rebalancearPerfilSemanal($mes, $anio, $ctx, $perfilSemanal, $turnoOpcionesPorNumero, $perfilObjetivo, $turnosPorPuestoFecha, $warnings);
                $warnings[] = 'Rebalanceo de perfil semanal completado.';

                $this->rescateFinalPerfilSemanal($mes, $anio, $perfilObjetivo, $warnings, $ctx, $turnosPorPuestoFecha);
                $warnings[] = 'Rescate final de perfil semanal completado.';

                $this->verificarPerfilSemanalObjetivoEstricto($mes, $anio, $perfilObjetivo, $warnings, $inviablesPerfil);
                if (!empty($inviablesPerfil)) {
                    throw new RuntimeException('Perfil semanal obligatorio no alcanzado. Trabajadores/semanas incumplidas: ' . implode(' | ', array_slice($inviablesPerfil, 0, 10)));
                }
            } else {
                $warnings[] = 'Perfil obligatorio: validación sin rebalanceo (en construcción).';
            }

            // ════════════════════════════════════════════════════════════════
            // PASO 3 — DÍAS LIBRES (AL FINAL)
            // Son obligatorios y deben seguir generándose aunque el proceso esté
            // en modo rápido, con un límite de tiempo acotado para no arrastrar el cierre.
            // ════════════════════════════════════════════════════════════════
            ...
            $stmtCheckTurnoDia = $this->db->prepare(
                "SELECT COUNT(*) as c FROM turnos_asignados WHERE trabajador_id = ? AND fecha = ? AND estado IN ('programado','activo')"
            );
            $stmtCheckDiaEspecial = $this->db->prepare(
                "SELECT COUNT(*) as c FROM dias_especiales WHERE trabajador_id = ? AND tipo IN ('L','L8','LC') AND ? BETWEEN fecha_inicio AND COALESCE(fecha_fin, fecha_inicio) AND estado IN ('programado','activo')"
            );
            ...
            $semaIdxLibres = 0;
            foreach ($semanasShuffled as $semana) {
                // Commit cada 2 semanas para evitar lock timeout
                if ($semaIdxLibres > 0 && ($semaIdxLibres % 2) === 0 && $manejaTransaccion && $this->db->inTransaction()) {
                    $this->db->commit();
                    error_log('[COMMIT] Commit después de semana libres #' . $semaIdxLibres);
                    $this->db->beginTransaction();
                }
                $semaIdxLibres++;
                ...
                if ((microtime(true) - $tiempoInicioDiasLibres) > $tiempoLimiteDiasLibres) {
                    $warnings[] = 'Se interrumpió la generación de días libres por límite de tiempo de la fase final.';
                    break;
                }
                ...
                foreach ($trabajadoresShuffled as $trab) {
                    ...
                    try {
                        // Doble verificación: si el trabajador ya tiene un turno ese día
                        // o ya tiene un día especial programado, evitamos insertar el libre.
                        $stmtCheckTurnoDia->execute([(int)$trab['id'], $mejorDia]);
                        $hasTurno = (int)($stmtCheckTurnoDia->fetch(PDO::FETCH_ASSOC)['c'] ?? 0) > 0;
                        $stmtCheckDiaEspecial->execute([(int)$trab['id'], $mejorDia]);
                        $hasDia = (int)($stmtCheckDiaEspecial->fetch(PDO::FETCH_ASSOC)['c'] ?? 0) > 0;
                        if ($hasTurno || $hasDia) {
                            $libresErrores[] = ['trabajador' => $trab['nombre'], 'semana' => $semana['lunes'], 'error' => 'Conflicto: ya tiene turno o día especial en ' . $mejorDia];
                            // marcar en ctx si hay turno para evitar futuros intentos
                            if ($hasTurno) {
                                $ctx['asignadosPorDia'][$mejorDia][$trab['id']][] = 'X';
                            }
                            continue;
                        }
                        ...
                        // Limpiar caches de disponibilidad para esta fecha
                        try {
                            $this->trabajadores->limpiarCacheFecha($mejorDia);
                        } catch (Throwable $e) {
                            error_log('[WARN] limpiarCacheFecha falló para ' . $mejorDia . ': ' . $e->getMessage());
                        }
            ...
            // Si perfil es obligatorio, validar estrictamente (DESACTIVADO TEMPORALMENTE)
            if (!empty($opciones['perfil_obligatorio']) && false) {
                // $this->verificarPerfilSemanalObjetivoEstricto($mes, $anio, $perfilObjetivo, $warnings, $inviablesPerfil);
                // if (!empty($inviablesPerfil)) {
                //     throw new RuntimeException('Perfil semanal obligatorio no alcanzado. Trabajadores/semanas incumplidas: ' . implode(' | ', array_slice($inviablesPerfil, 0, 5)));
                // }
            }
            ...
            if (PHP_SAPI === 'cli') {
                $tiempoTotal = isset($inicioAsignacion) ? (microtime(true) - $inicioAsignacion) : null;
                $metrics1 = "[METRICS] tiempo_total=" . ($tiempoTotal !== null ? number_format($tiempoTotal, 2) . 's' : 'NA') . ", asignaciones=" . count($asignaciones) . ", errores=" . count($errores) . ", libres_asignados=" . count($libresAsignados) . ", libres_errores=" . count($libresErrores);
                $metrics2 = "[METRICS] fallback_nivel1=" . ($fallbackStats[1] ?? 0) . ", nivel2=" . ($fallbackStats[2] ?? 0) . ", nivel3=" . ($fallbackStats[3] ?? 0) . ", nivel4=" . ($fallbackStats[4] ?? 0);
                echo $metrics1 . "\n";
                echo $metrics2 . "\n";
                error_log($metrics1);
                error_log($metrics2);
                $logPath = dirname(__DIR__, 2) . '/logs/assignment_run.log';
                @file_put_contents($logPath, $metrics1 . PHP_EOL, FILE_APPEND);
                @file_put_contents($logPath, $metrics2 . PHP_EOL, FILE_APPEND);
            }
    ...
    private function verificarPerfilSemanalObjetivoEstricto($mes, $anio, $perfilObjetivo, &$warnings = [], &$inviables = []) {
        ...
        foreach ($perfil as $trabajadorId => $semanasTrab) {
            foreach ($semanasTrab as $semKey => $p) {
                $ok = true;
                $ok = $ok && ((int)$p['h8'] === (int)($perfilObjetivo['semanal_8h'] ?? 3));
                $ok = $ok && ((int)$p['h7'] === (int)($perfilObjetivo['semanal_7h'] ?? 2));
                $ok = $ok && ((int)$p['h4'] === (int)($perfilObjetivo['semanal_4h'] ?? 1));
                $ok = $ok && ((int)$p['turnos'] === 6);
                $ok = $ok && (abs((float)$p['total_horas'] - (float)($perfilObjetivo['semanal_horas'] ?? 42.0)) < 0.01);
                if ($ok) continue;

                $faltantes = [
                    'h8'     => max(0, (int)($perfilObjetivo['semanal_8h'] ?? 3) - (int)$p['h8']),
                    'h7'     => max(0, (int)($perfilObjetivo['semanal_7h'] ?? 2) - (int)$p['h7']),
                    'h4'     => max(0, (int)($perfilObjetivo['semanal_4h'] ?? 1) - (int)$p['h4']),
                    'turnos' => max(0, 6 - (int)$p['turnos']),
                ];

                $excesos = [
                    'h8' => max(0, (int)$p['h8'] - (int)($perfilObjetivo['semanal_8h'] ?? 3)),
                    'h7' => max(0, (int)$p['h7'] - (int)($perfilObjetivo['semanal_7h'] ?? 2)),
                    'h4' => max(0, (int)$p['h4'] - (int)($perfilObjetivo['semanal_4h'] ?? 1)),
                ];
                ...
                $detalle = sprintf(
                    '%s semana %s: h8=%d h7=%d h4=%d turnos=%d horas=%.1f (faltantes: h8=%d h7=%d h4=%d)',
                ...
                // Si tiene faltantes, es un error en modo estricto
                if ($sumFaltantesBloque > 0 || (int)$p['turnos'] < 6) {
                    $errores[] = $detalle;
                    if (count($errores) >= 12) {
                        break 2;
                    }
    ...
    private function verificarPerfilSemanalObjetivoRapido($mes, $anio, $perfilObjetivo, $perfilSemanal, $trabajadores, &$warnings = [], &$inviables = []) {
        ...
        foreach ($nombres as $trabajadorId => $nombre) {
            ...
            foreach ($semanasCompletas as $semKey) {
                ...
                $ok = true;
                $ok = $ok && ((int)($perfil['h8'] ?? 0) === (int)($perfilObjetivo['semanal_8h'] ?? 3));
                $ok = $ok && ((int)($perfil['h7'] ?? 0) === (int)($perfilObjetivo['semanal_7h'] ?? 2));
                $ok = $ok && ((int)($perfil['h4'] ?? 0) === (int)($perfilObjetivo['semanal_4h'] ?? 1));
                $ok = $ok && ($turnos === 6);
                $ok = $ok && (abs((float)($perfil['total_horas'] ?? 0.0) - (float)($perfilObjetivo['semanal_horas'] ?? 42.0)) < 0.01);
                if ($ok) continue;
    ...
    private function rebalancearPerfilSemanal($mes, $anio, &$ctx, &$perfilSemanal, $turnoOpcionesPorNumero, $perfilObjetivo, &$turnosPorPuestoFecha, &$warnings) {
        ...
        $ocupadoPorPuestoNumeroFecha = &$turnosPorPuestoFecha;
        ...
        foreach ($ctx['todosActivos'] as $trab) {
            ...
            foreach ($semanasCompletas as $sem) {
                ...
                $need7 = max(0, (int)$perfilObjetivo['semanal_7h'] - (int)($perfil['h7'] ?? 0));
                $extra8 = max(0, (int)($perfil['h8'] ?? 0) - (int)$perfilObjetivo['semanal_8h']);
                    ...
                    foreach ($asigsSemana as $a) {
                        ...
                        foreach (($puestosL4PorBase[$base] ?? []) as $pL4) {
                            foreach ($turnosL4Ids as $turnoL4Id) {
                                ...
                                if ($this->tieneTurnoL4EnFechaYTurnoId((int)$pL4['id'], $a['fecha'], $turnoL4Id, $ocupadoPorPuestoNumeroFecha)
                                    || $this->cantidadTurnosL4EnFechaYBase((int)$pL4['id'], $a['fecha'], $base, $ocupadoPorPuestoNumeroFecha) >= 2) {
                                    continue;
                                }
                ...
                while (
                    ...
                    $need8 = (int)$perfilObjetivo['semanal_8h'] - (int)($perfilSemanal[$trabId][$semKey]['h8'] ?? 0);
                    $need7 = (int)$perfilObjetivo['semanal_7h'] - (int)($perfilSemanal[$trabId][$semKey]['h7'] ?? 0);
                    $prefer8 = $need8 > 0;
                        ...
                        if ($prefer8) {
                            foreach ($puestos8h as $p8) {
                                ...
                                foreach ([1,2,3] as $numBase8) {
                                    ...
                                    $keyOcup = (int)$p8['id'] . '|' . $numBase8 . '|' . $fecha;
                                    $ocup = !empty($ocupadoPorPuestoNumeroFecha[$keyOcup]) ? 1 : 0;
                                    if ($ocup > 0) continue;
                        ...
                        if (!$agregado && $need7 > 0) {
                            foreach ($puestos7h as $p7) {
                                ...
                                foreach ([1,2] as $numBase) {
                                    ...
                                    $keyOcup = (int)$p7['id'] . '|' . $numBase . '|' . $fecha;
                                    $ocup = !empty($ocupadoPorPuestoNumeroFecha[$keyOcup]) ? 1 : 0;
                                    if ($ocup > 0) continue;
                        ...
                        foreach ([1,2] as $base) {
                            ...
                            foreach (($puestosL4PorBase[$base] ?? []) as $pL4) {
                                foreach ($turnosL4Ids as $turnoL4Id) {
                                    ...
                                    if ($this->tieneTurnoL4EnFechaYTurnoId((int)$pL4['id'], $fecha, $turnoL4Id, $ocupadoPorPuestoNumeroFecha)
                                        || $this->cantidadTurnosL4EnFechaYBase((int)$pL4['id'], $fecha, $base, $ocupadoPorPuestoNumeroFecha) >= 2) {
                                        continue;
                                    }
    ...
    private function rescateFinalPerfilSemanal($mes, $anio, $perfilObjetivo, &$warnings, &$ctx, &$turnosPorPuestoFecha) {
        $semanas = $this->calcularSemanas($mes, $anio);
        $semanasCompletas = [];
        foreach ($semanas as $sem) {
            if ((int)date('n', strtotime($sem['lunes'])) !== (int)$mes) continue;
        ...
        foreach ($pendientes7 as $p7) {
            $ok = $this->intentarAgregar7hSemana((int)$p7['trabajador_id'], $p7['semana'], $mes, $ctx, $turnosPorPuestoFecha);
            if (!$ok) {
        ...
        foreach ($pendientes4 as $p4) {
            $ok = $this->intentarAgregar4hSemana((int)$p4['trabajador_id'], $p4['semana'], $mes, $ctx, $turnosPorPuestoFecha);
            if (!$ok) {
    ...
    private function intentarAgregar7hSemana($trabajadorId, $semana, $mes, &$ctx, &$turnosPorPuestoFecha) {
        $turno7PorNumero = [1 => null, 2 => null];
        $stmtCfg = $this->db->prepare(
            "SELECT id, numero_turno, horas_laborales
             FROM configuracion_turnos
        ...
        for ($d = 0; $d <= 6; $d++) {
            ...
            foreach ($puestos7h as $p7) {
                ...
                foreach ([1,2] as $numBase) {
                    ...
                    $turno7Id = (int)($turno7PorNumero[$numBase] ?? 0);
                    if ($turno7Id <= 0) continue;
                    if ($this->estaPuestoOcupado((int)$p7['id'], $numBase, $fecha, $turnosPorPuestoFecha)) continue;
    ...
    private function intentarAgregar4hSemana($trabajadorId, $semana, $mes, &$ctx, &$turnosPorPuestoFecha) {
        $turnoL4PorBase = [1 => [], 2 => []];
        $stmtCfg = $this->db->prepare(
            "SELECT id, numero_turno, horas_laborales
             FROM configuracion_turnos
            ...
            foreach ([1,2] as $base) {
                ...
                foreach (($puestosL4PorBase[$base] ?? []) as $pL4) {
                    foreach ($turnosL4 as $turnoL4Id) {
                        ...
                        if ($this->tieneTurnoL4EnFechaYTurnoId((int)$pL4['id'], $fecha, $turnoL4Id, $turnosPorPuestoFecha)
                            || $this->cantidadTurnosL4EnFechaYBase((int)$pL4['id'], $fecha, $base, $turnosPorPuestoFecha) >= 2) {
                            continue;
                        }
    ...
    private function buscarCandidatosLibre($trabajadorId, $semana, $mes, $tsUltimoLibre, $cargaPorFecha, $maxLibresDia, $ctx = [], $libresAsignadosPorFecha = []) {
        for ($nivel = 1; $nivel <= 3; $nivel++) {
            ...
            for ($d = 0; $d <= 6; $d++) {
                ...
                if ($this->tieneDiaEspecialEnFecha($trabajadorId, $fechaDia, $ctx)) continue;
    ...
    private function tieneDiaEspecialEnFecha($trabajadorId, $fecha, $ctx) {
        if (empty($trabajadorId) || !$fecha || empty($ctx['diasEspeciales'])) {
            return false;
        }

        foreach ($ctx['diasEspeciales'] as $de) {
            if ((int)($de['trabajador_id'] ?? 0) !== (int)$trabajadorId) {
                continue;
            }
            if (!isset($de['fecha_inicio'])) {
                continue;
            }
            $fechaInicio = $de['fecha_inicio'];
            $fechaFin = isset($de['fecha_fin']) && $de['fecha_fin'] ? $de['fecha_fin'] : $fechaInicio;
    ...
    private function elegirOpcionTurnoParaPerfil($opcionesTurno, $perfil, $perfilObjetivo, $bloqueObjetivoPuesto = null) {
        ...
        $perfilTotal = (float)($perfil['total_horas'] ?? 0.0);
        $maxHorasSemanal = (float)($perfilObjetivo['semanal_horas'] ?? 42.0);
        $need8 = max(0, (int)($perfilObjetivo['semanal_8h'] ?? 3) - (int)($perfil['h8'] ?? 0));
        $need7 = max(0, (int)($perfilObjetivo['semanal_7h'] ?? 2) - (int)($perfil['h7'] ?? 0));
        $need4 = max(0, (int)($perfilObjetivo['semanal_4h'] ?? 1) - (int)($perfil['h4'] ?? 0));
        $excess8 = max(0, (int)($perfil['h8'] ?? 0) - (int)($perfilObjetivo['semanal_8h'] ?? 3));
        $excess7 = max(0, (int)($perfil['h7'] ?? 0) - (int)($perfilObjetivo['semanal_7h'] ?? 2));
        $excess4 = max(0, (int)($perfil['h4'] ?? 0) - (int)($perfilObjetivo['semanal_4h'] ?? 1));

        // Si ya completó las horas semanales, no agregar más
        if ($perfilTotal >= $maxHorasSemanal) {
            return null;
        }

        // Si ya tiene los 6 turnos objetivos (3x8 + 2x7 + 1x4), no agregar más
        $turnosActuales = (int)($perfil['h8'] ?? 0) + (int)($perfil['h7'] ?? 0) + (int)($perfil['h4'] ?? 0);
        if ($turnosActuales >= 6) {
            return null;
        }

        // NORMA OBLIGATORIA: Rechazar opciones que hagan EXCEDER límites
        // Filtrar por bloque objetivo del puesto si está especificado
        if ($bloqueObjetivoPuesto === 'h8') {
            if ($excess8 > 0) return null; // Ya tiene suficientes 8h
            if (!empty($ops8)) {
                $ordenarPorId($ops8);
                return $ops8[0];
            }
        ...
        if ($bloqueObjetivoPuesto === 'h7') {
            if ($excess7 > 0) return null; // Ya tiene suficientes 7h
            if (!empty($ops7)) {
                $ordenarPorId($ops7);
                return $ops7[0];
            }
        ...
        // Seleccionar en orden de prioridad para cumplir 3x8h + 2x7h + 1x4h
        // Prioridad: 8h -> 7h -> 4h
        // 1. Cubrir 8h (si hace falta y no hay exceso)
        if ($need8 > 0 && !empty($ops8) && $excess8 === 0) {
            // asegurar que añadir 8h no exceda horas semanales
            foreach ($ops8 as $op) {
                $horasOp = (float)($op['horas'] ?? 0);
                if (($perfilTotal + $horasOp) <= $maxHorasSemanal + 0.001) {
                    $ordenarPorId($ops8);
                    return $ops8[0];
                }
            }
        ...
        // 2. Cubrir 7h (si hace falta y no hay exceso)
        if ($need7 > 0 && !empty($ops7) && $excess7 === 0) {
            foreach ($ops7 as $op) {
                $horasOp = (float)($op['horas'] ?? 0);
                if (($perfilTotal + $horasOp) <= $maxHorasSemanal + 0.001) {
                    $ordenarPorId($ops7);
        ...
        // 3. Cubrir 4h (si hace falta y no hay exceso)
        if ($need4 > 0 && !empty($ops4) && $excess4 === 0) {
            foreach ($ops4 as $op) {
                $horasOp = (float)($op['horas'] ?? 0);
                if (($perfilTotal + $horasOp) <= $maxHorasSemanal + 0.001) {
                    $ordenarPorId($ops4);
        ...
        // 4. Si aún hay horas pero no necesita completar bloques específicos,
        //    solo permitir si hay lugar sin exceder límites
        if ($perfilTotal < $maxHorasSemanal && $turnosActuales < 6) {
            if ($need8 > 0 && !empty($ops8)) {
                $ordenarPorId($ops8);
                return $ops8[0];
            }
            if ($need7 > 0 && !empty($ops7)) {
                $ordenarPorId($ops7);
                return $ops7[0];
            }
            if ($need4 > 0 && !empty($ops4)) {
                $ordenarPorId($ops4);
                return $ops4[0];
            }
        }
    ...
    private function ordenarPuestosPorEscasezYPrioridad($puestos, $turnoNumero, $turnoIdBase, $fecha, &$ctx, &$conteoTurnos) {
        ...
        usort($puestosOrdenados, function($a, $b) use ($turnoNumero) {
    ...
    private function seleccionarMejorCandidatoSemanal($disponibles, $fecha, $turnoNumero, $puestoId, $codigoPuesto, $bloqueObjetivoPuesto, $turnoOpcionesPorNumero, $perfilSemanal, $perfilObjetivo, $maxHorasSemanalOperativo, $conteoTurnos, &$ctx) {
        ...
        $modoRapido = !empty($ctx['modoRapido']);
        ...
        foreach ($disponibles as $trab) {
            ...
            $item = [
                ...
                'conteo'     => $conteoBase,
                'score'      => $this->calcularScoreCandidatoCobertura($id, $turnoNumero, $codigoPuesto, $turnoHoras, $perfil, $conteoBase, $ctx, $penalidad, $fecha),
            ];

            // Usar el límite semanal correcto (42h, no mensual)
            if ($totalProy <= 42.001) {
                $elegibles[] = $item;
            }
        }

        $ordenar = function(&$lista) use ($turnoNumero, &$ctx) {
        ...
        if (!empty($elegibles)) {
            ...
            // En el camino principal del mes, el cuello de botella ya no es la
            // puntuación del candidato sino la validación repetida por candidato.
            // Como la lista ya viene filtrada por disponibilidad y restricciones del
            // día, se usa un shortlist muy pequeño y se elige el mejor elemento sin
            // volver a consultar la base de datos por cada opción.
            $topK = array_slice($elegibles, 0, (($ctx['modoRapido'] ?? false) ? 8 : 12));
            $candidatosValidos = [];
            foreach ($topK as $elegible) {
                ...
                $escRel = $this->getEscasezRelativaCandidato($trabajadorId, $turnoNumero, $ctx);
                $candidatosValidos[] = ['item' => $elegible, 'escRel' => $escRel];
            }

            if (empty($candidatosValidos)) {
            ...
            if (!empty($candidatosValidos)) {
                usort($candidatosValidos, function($a, $b) {
                    if ($a['escRel'] !== $b['escRel']) return $a['escRel'] <=> $b['escRel'];
                    return $a['item']['score'] <=> $b['item']['score'];
                });
                $sel = $candidatosValidos[0]['item'];
                return [
                    'trabajador' => $sel['trabajador'],
                    'turno_id'   => (int)$sel['turno_id'],
                    'turno_horas'=> $sel['turno_horas'],
                ];
            }
        }
    ...
    private function seleccionarCandidatoCobertura($disponibles, $fecha, $turnoNumero, $puestoId, $bloqueObjetivoPuesto, $turnoOpcionesPorNumero, $perfilSemanal, $perfilObjetivo, $maxHorasSemanalCobertura, $conteoTurnos, &$ctx, $codigoPuesto = '') {
        ...
        foreach ($candidatos as $candidato) {
            ...
            return [
                ...
                'turno_id'   => (int)$candidato['turno_id'],
    ...
    private function calcularScoreCandidatoCobertura($trabajadorId, $turnoNumero, $codigoPuesto, $turnoHoras, $perfil, $conteoBase, &$ctx, $penalidadBase = 0, $fecha = null) {
        ...
        // Hacer la escasez más influyente en el score para priorizar candidatos escasos
        $bonusEscasez = ($pesoEscasezTurno / $opcionesTurno) * $factorCritico * 1.2;
        $bonusGlobal = (60.0 / $opcionesReales) * $factorCritico * 1.5;
        ...
        if (($perfil['total_horas'] ?? 0.0) <= 0.001) {
            $score -= 4.0;
        }
        if ($fecha && (int)date('N', strtotime($fecha)) === 7) {
            $score -= 6.0;
        }
    ...
    private function optimizarCoberturaFinal($mes, $anio, $puestos, $puestosNocturnos, $puestosL4Turno, $turnoIdPorNumero, $turnoOpcionesPorNumero, $perfilObjetivo, &$ctx, &$conteoTurnos, &$perfilSemanal, &$turnosPorPuestoFecha, &$asignaciones) {
        ...
        // Mantener esta fase muy acotada para no arrastrar el proceso completo
        // del mes. El algoritmo principal ya cubre la mayoría de los huecos.
        $maxSegundos = 12.0;

        // Si hay demasiados huecos, evitamos entrar en la optimización final
        // porque es mucho más costosa que el paso principal y no vale la pena
        // para una mejora de cobertura que puede tardar mucho.
        if ($cantidadInicial > 24) {
    ...
    private function intentarReasignacionLocalHueco($hueco, $turnoOpcionesPorNumero, $perfilObjetivo, &$ctx, &$conteoTurnos, &$perfilSemanal, &$turnosPorPuestoFecha, &$asignaciones, &$detalleMovimiento = [], &$diagnostico = null) {
        ...
        foreach ($asignacionesDia as $asigDia) {
            ...
            $disponiblesReemplazo = $this->filtrarDisponiblesPorRestriccionesObligatorias(
                ...
                $puestoMover,
                $turnoOpcionesPorNumero[$turnoMoverNum] ?? [],
                $fecha
            );
    ...
    private function seleccionarCandidatoExacto($disponibles, $fecha, $turnoNumero, $puestoId, $codigoPuesto, $turnoIdExacto, $turnoHoras, $perfilSemanal, $perfilObjetivo, $conteoTurnos, &$ctx) {
        ...
        foreach ($disponibles as $trab) {
            ...
            $totalActual = (float)($perfil['total_horas'] ?? 0.0);
            if (($totalActual + (float)$turnoHoras) > 48.001) continue;
